(function() {
				...
				fetch(`/api/share/invite/users?name=` + encodeURIComponent(name))
					...
					.then(data => {
						...
						data.forEach(m => {
							...
							span.textContent = m.hname + (m.requested ? ' (나에게 요청한 유저)' : '');
	...
	function showCompleted(actionContainer) {
		actionContainer.innerHTML = '';  // 기존 버튼들 제거

		const doneBtn = document.createElement('button');
		doneBtn.className = 'invite-btn disabled';
		doneBtn.textContent = '초대완료';
